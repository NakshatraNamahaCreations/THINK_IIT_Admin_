...
const QuestionDistributionSidebar = ({
  ...
  return (
    ...
    chapter.topic?.reduce((tAcc, topic) => {
      const topicName = topic.topicName?.trim();
      const matchedTopic = Object.keys(pickedQuestions?.[sectionId] || {}).find(
        (key) => key.trim().toLowerCase() === topicName?.toLowerCase()
      );

      const topicCount = matchedTopic
        ? Object.keys(pickedQuestions[sectionId][matchedTopic]).length
        : 0;

      // STORE IN LOCAL STORAGE HERE
      const subjectKey = `${sectionId}_${subject.subjectName}`;
      const chapterKey = chapter.chapterName;

      const existing = JSON.parse(localStorage.getItem("questionSummary") || "{}");
      const updated = {
        ...existing,
        [subjectKey]: {
          ...(existing[subjectKey] || {}),
          [chapterKey]: topicCount,


        },
      };
      localStorage.setItem("questionSummary", JSON.stringify(updated));

      return tAcc + topicCount;
    }, 0)
  );
}, 0)
}
